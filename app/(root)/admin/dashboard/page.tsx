...
import { RecentSale, RecentSaleApiResponse } from '@/app/type/recentSale';
...
function DashboardPage() {
  ...
  const { data: revenueData, isLoading: loadingRevenue } = useFetchTotalRevenue<{ gain: number }>();
  ...
  const columns = [
    {
      ...
      key: "id",
      dataIndex: "id",
      render: (value: string, record: RecentSale, index: number) => (
  ...
  return (
    <div className='lg:pr-12'>
      <div className="flex flex-col md:flex-row items-center gap-3 lg:gap-12 mb-8">
        {stats.map((stat, index) => (
          <div
            key={index}
            className="w-full sm:w-[300px] h-[150px] flex flex-col justify-between hover:shadow-lg transition delay-150 duration-300 ease-in-out hover:-translate-y-1 hover:scale-10% border-2 p-3 rounded border-[#E28E34]"
          >
            <div className="flex justify-between items-center">
              <p className='text-gray-500'>{stat.title}</p>
              <p>{stat.icon}</p>
            </div>
            <div className={`text-2xl font-bold ${stat.highlight ? "text-green-500" : ""}`}>
              {stat.value}
            </div>
